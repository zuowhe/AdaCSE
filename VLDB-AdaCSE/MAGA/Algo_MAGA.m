function Algo_MAGA(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'MAGA';         % 记录运行算法的名称，便于后续结果的存储
...
for T = 1:trial
    ...
    % 主算法调用
    [dag, eoer{T}(1,4),conv(T),eoer{T}(1,6)] = MAGA_OnlyBIC_process(SuperStructure,TrainData{T},N,M,MP,scoring_fn,bnet,tour,saves_result_file);

    
    % 整理结果
    eoer{T}(1,5) = toc(tStart);                                             % toc
    bian = size(bnet.dag,1);
    dag_suan = reshape(dag{1}',bian,bian)';
    my_cell = {dag_suan}; 
    [eoer{T}(1,1),eoer{T}(1,2),eoer{T}(1,3),eoer{T}(1,8),eoer{T}(1,7),TP,TP2,FN,FP,TN]= eval_dags_adjust(my_cell,bnet.dag,1);
