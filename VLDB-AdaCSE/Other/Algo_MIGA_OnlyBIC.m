function [onedata_avg_result, onedata_std_result] = Algo_MIGA_OnlyBIC(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'MIGA_0.01';         % 记录运行算法的名称，便于后续结果的存储
...
%     SuperStructure = he_get_CI_test(TrainData{T},bnet,tol);
    
    % 主算法调用
    [dag,eoer{T}(1,4),conv(T),eoer{T}(1,6)] = ...
        MIGA_OnlyBIC_process(SuperStructure,TrainData{T},N,M,MP,scoring_fn,bnet,tour,saves_result_file);
...
onedata_avg_result = [eoer_avg(1),eoer_avg(7),eoer_avg(5)];
onedata_std_result = [eoer_std(1),eoer_std(7),eoer_std(5)];
end
