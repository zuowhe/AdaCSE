function Algo_MYGA_muti(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'MYGA_muti';         % 记录运行算法的名称，便于后续结果的存储
...
fprintf('Iter   F1 Score   SHD    Bayes Score   DBSCAN_time   Exe Time  #Generations TP  TP2  FN  FP  TN   score_chushi \n');
%% =========================  GA  =================================
for T = 1:trial
    % 拼接存储单次收敛过程的CSV文件的名称
...
%     data1 = Kmeans_myga(TrainData{T});    % kmeans提取
%     [data1, execution_time]  = Random_myga_muti(TrainData{T});  % 随机提取
    [data1, execution_time]  = DBSCAN_myga2_muti(TrainData{T});
%     [data1, dbscan_time] = DBSCAN_myga2(TrainData{T});
    data_all = TrainData{T};
    
    % 主算法调用
%     [dag,eoer{T}(1,4),conv(T),eoer{T}(1,6)] = ...
%         MYGA_muti_initialization(SuperStructure,data1,data_all,N,M,MP,scoring_fn,bnet,tour,saves_result_file);
        [dag,eoer{T}(1,4),conv(T),eoer{T}(1,6)] = ...
        MYGA_muti_vote(SuperStructure,data1,data_all,N,M,MP,scoring_fn,bnet,tour,saves_result_file);
    
    % 整理结果
    eoer{T}(1,5) = toc(tStart);                                             % toc
    eoer{T}(1,10) = execution_time; 
    [eoer{T}(1,1),eoer{T}(1,2),eoer{T}(1,3),eoer{T}(1,8),eoer{T}(1,7),TP,TP2,FN,FP,TN]= eval_dags_adjust(dag,bnet.dag,1);
    
    fprintf('%4d  %9.5f    %3.1f   %11.3f  %11.3f %11.3f      %4d       %4d%4d%4d%4d%4d   %11.3f\n',...
        T,eoer{T}(1),eoer{T}(7),eoer{T}(4),eoer{T}(1,10),eoer{T}(5),eoer{T}(6),TP,TP2,FN,FP,TN,eoer{T}(1,9));
        %10 01\TP完全正确的边数\TP2对一半的边数\%FN完全错误的边数\
        %00\FP完全错误的边数\%TN完全正确的边数
    
    saved_file = fopen(saves_filename_result,'a+');         % a+:文件若不存在，自动创建
    fprintf(saved_file,'%4d,%9.5f,%3.1f,%11.3f,%11.3f,%11.3f,%4d,%4d,%4d,%4d,%4d,%4d,%11.3f\n',...
        T,eoer{T}(1),eoer{T}(7),eoer{T}(4),eoer{T}(1,10),eoer{T}(5),eoer{T}(6),TP,TP2,FN,FP,TN,eoer{T}(1,9));
end
...
fprintf(saved_file,'Avg,%9.5f,%5.3f,%11.3f,%11.3f,%11.3f,%7.3f,%11.3f\n',...
        eoer_avg(1),eoer_avg(7),eoer_avg(4),eoer_avg(10),eoer_avg(5),eoer_avg(6),eoer_avg(9));
fprintf(saved_file,'Std,%9.5f,%5.3f,%11.3f,%11.3f,%11.3f,%7.3f,%11.3f\n',...
        eoer_std(1),eoer_std(7),eoer_std(4),eoer_std(10),eoer_std(5),eoer_std(6),eoer_std(9));
fclose(saved_file);
