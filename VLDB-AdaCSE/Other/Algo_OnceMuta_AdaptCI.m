function Algo_OnceMuta_AdaptCI(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'MIGA2_RandomX_AdaptCI';         % 记录运行算法的名称，便于后续结果的存储
...
%     SuperStructure = get_CI_test(TrainData{T},bnet,tol);
    ...
    [dag,eoer{T}(1,4),conv(T),eoer{T}(1,6)] = ...
        MYGA_OnceMuta_AdaptCI_process(TrainData{T},N,M,MP,scoring_fn,bnet,tour,saves_result_file,p_value);
