function Algo_MYGA_kmeans(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'MYGA_kmeans';         % 记录运行算法的名称，便于后续结果的存储
...
for T = 1:trial
    ...
    [data1, dbscan_time] = Kmeans_myga(TrainData{T});    % kmeans提取
%     [data1, dbscan_time] = Random_myga(TrainData{T});  % 随机提取
%     [data1, dbscan_time] = DBSCAN_myga2(TrainData{T});
    data_all = TrainData{T};
    
    % 主算法调用
    [dag,eoer{T}(1,4),conv(T),eoer{T}(1,6)] = ...
        MYGA_ga_process(SuperStructure,data1,data_all,N,M,MP,scoring_fn,bnet,tour,saves_result_file);
    
    % 整理结果
    eoer{T}(1,5) = toc(tStart);                                             % toc
    eoer{T}(1,10) = dbscan_time; 
