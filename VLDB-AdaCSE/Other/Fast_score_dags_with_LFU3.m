function [score, cache, node_scores] = Fast_score_dags_with_LFU3(data, ns, dags, varargin)
...
%   node_scores:    1 x n cell, each cell contains parent-set-score records
    ...
    for g = 1:NG
        for j = 1:n
            total_parents = total_parents + length(parents(dags, j));
        end
    end
...
function [cache, replaced] = cache_insert_or_update(cache, key, new_score)
    ...
    % 如果缓存未满，直接插入
    if length(cache.key) < cache.cache_size && ~isempty(cache.key)
        cache.key = [cache.key, {key}];
        cache.score = [cache.score, new_score];
        cache.freq = [cache.freq, 1];
    else
        % 否则使用 LFU 替换
        [~, idx] = min(cache.freq);
        cache.key(idx) = {key};
