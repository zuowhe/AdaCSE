function Algo_MMHC(Ds,BN_Name,N,M,bnet,trial)
%% CausalExplorer MMHC
AlgoName = 'MMHC';
...
for T = 1:trial
    ...
    tStart = tic;    
    % 
    rmpath(genpath('D:\博士文件\代码\BNSL_All\bnt-master\'));
    addpath('D:\博士文件\代码\BNSL_All\Algorithms\MMHC\causal_explorer');
    
%     trian_data = TrainData{T};
    input_data = TrainData{T}'-1;
    [dag_output,eoer{T}(1,6),~,~,~] = ...
        Causal_Explorer('MMHC',input_data,bnet.node_sizes,'MMHC',options,10,'BDeu');

    % 方案2：传数字数组（变量索引作为名称）
    var_names = num2cell(1:size(input_data, 2));
    [dag, scores] = mmhc_algorithm(input_data, var_names);
    

    
    dag = false(size(bnet.dag));
    dag(dag~=dag_output) = true;
    addpath(genpath('D:\博士文件\代码\BNSL_All\bnt-master\'));
    rmpath('D:\博士文件\代码\BNSL_All\Algorithms\MMHC\causal_explorer');
    
    dags{T} = dag;
    % 整理结果
    eoer{T}(1,5) = toc(tStart);                                             % toc
    [eoer{T}(1,1),eoer{T}(1,2),eoer{T}(1,3),eoer{T}(1,8),eoer{T}(1,7),TP,TP2,FN,FP,TN]= eval_dags_adjust(dags,bnet.dag,1);
