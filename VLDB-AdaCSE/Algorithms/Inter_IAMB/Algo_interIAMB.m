function Algo_interIAMB(Ds,BN_Name,N,M,bnet,trial)
%% CausalExplorer Inter-IAMB
...
% Inter-IAMB
    ...
    dag = false(bns,bns);
    for i = 1:bns
        MB = Causal_Explorer('interIAMB', input_data, i, bnet.node_sizes, 'mi', 0.05);
        if MB
            cpdag(i,MB) = 2;
            cpdag(MB,i) = 2;
            for j = 1:size(MB,2)
...
%     eoer{T}(1,7) = get_HD(dag{1,1},bnet.dag);        % HD
