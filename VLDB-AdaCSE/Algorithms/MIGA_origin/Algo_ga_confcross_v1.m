function Algo_ga_confcross_v1(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'MIGA_origin';         % 记录运行算法的名称，便于后续结果的存储
...
saves_filename_result = sprintf('[result]/%s/%s%s_%s_%s_%s.csv',...    % 拼接存储单组实验结果的CSV文件的名称
...
for T = 1:trial
    ...
    ss = get_CI_test(TrainData{T},bnet,tol);
    
    % 主算法调用
    [dag,eoer{T}(1,4),conv(T),eoer{T}(1,6)] = ...
        ga_confcross_v1(ss,TrainData{T},N,M,MP,scoring_fn,bnet,tour,saves_result_file);

    dag2file(dag, convergence_dir, AlgoName, BN_Name, Ds, N, M, T);  % 将算法学习到的结构保存到文件
...
% filename = sprintf('[result]/eoer_%s%s_%s_%s_%s_%s',net,num2str(Ds),num2str(N),num2str(M),num2str(MP),algo);
% save(filename,'eoer');
% fn = fieldnames(conv);

% filename = sprintf('[result]/conv_%s%s_%s_%s_%s_%s',net,num2str(Ds),num2str(N),num2str(M),num2str(MP),algo);
...
% fn = fieldnames(conv);
% for f=1:numel(fn)
%     conv_avg.(fn{f}) = sum(cat(M,conv.(fn{f})),M)/trial;
% end
% filename = sprintf('[result]/conv_%s%s_%s_%s_%s_%s',net,num2str(Ds),num2str(N),num2str(M),num2str(MP),algo);
% save(filename,'conv_avg');
