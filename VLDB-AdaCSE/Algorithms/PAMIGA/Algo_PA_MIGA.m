function Algo_PA_MIGA(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'PA_MIGA';         % 记录运行算法的名称，便于后续结果的存储
...
%% ===================并行GA==================
[All_dag,eoer,every_best] = PAtrial(tour,scoring_fn,MP,M,N,tol,bnet,trial,TrainData);
%% ==================实验结果=================
for T = 1:trial
    % 拼接存储单次收敛过程的CSV文件的名称===网络名称和训练集大小_种群规模_最大迭代_算法名称-第几次实验
    saves_best_file = sprintf('[result]/%s/Convergence_Behavior/%s%s_%s_%s_%s-%s.csv',...
    NowDate,BN_Name,num2str(Ds),num2str(N),num2str(M),AlgoName,num2str(T,'%02d'));
    saved_convsfile=fopen(saves_best_file,'w');
    once_convs = every_best{T,1};
    for i = 1:M
        fprintf(saved_convsfile,'%d\n',once_convs(i)); 
    end
    fclose(saved_convsfile);
    % 整理结果
    dag2 = All_dag(T);
    [eoer{T}(1,1),eoer{T}(1,2),eoer{T}(1,3),eoer{T}(1,8),eoer{T}(1,7),TP,TP2,FN,FP,TN]= eval_dags_adjust(dag2{1,1},bnet.dag,1);
    % 在命令行输出
