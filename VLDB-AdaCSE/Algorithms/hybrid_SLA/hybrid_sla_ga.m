function [dag,sc_star,conv,last_gen] = hybrid_sla_ga(ss,data,N,M,MP,scoring_fn,bnet,saved_filename)
...
for i=1:M
    ...
    if ~isempty(find(norm_score,1)) % all individuals are not the same
        [p2,~] = CHC_selection(N,pop,score);
        p2 = CHC_crossover(N,p2,l_map);
        p2(N+1:N2) = make_dag_naive(p2(N+1:N2),MP);
    else
        p2 = pop;
    end
    
    pop = CHC_mutation(N2,l_map,p2);
...
%     [score, cache] = score_dags(data,ns,p,'scoring_fn',scoring_fn,'cache',cache);
    ...
    'max_cache_size', max_cache_size);
    total_overwrite = total_overwrite + once_cache_hits;
%     [score, cache] = score_dags_norecover(data, ns, pop, 'cache',cache);
