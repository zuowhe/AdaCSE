function Algo_CIMAGA(Ds,BN_Name,N,M,MP,tour,bnet,trial,scoring_fn)
%% naive MIqh / confidence MIqh 互信息去环 / 置信度交叉
AlgoName = 'CI-MAGA';         % 记录运行算法的名称，便于后续结果的存储
...
for T = 1:trial
    ...
    % 主算法调用
    [dag, eoer{T}(1,4),conv(T),eoer{T}(1,6)] = CIMAGA_process(SuperStructure,TrainData{T},N,M,MP,scoring_fn,bnet,tour,saves_result_file);
